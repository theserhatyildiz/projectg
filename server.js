...
app.use(helmet());
app.use(cors({
  origin: 'https://galwinapp-c654a544b729.herokuapp.com/', // Allow requests from this origin
...
app.post('/refresh-token', async (req, res) => {
  ...
  try {
    ...
    if (timeRemaining < 24 * 60 * 60 * 1000) { // Less than 1 day remaining
      const newExpiryDate = new Date(now + 7 * 24 * 60 * 60 * 1000); // Extend by 7 more days
      const newRefreshToken = new refreshTokenModel({
        token: crypto.randomBytes(40).toString('hex'),
